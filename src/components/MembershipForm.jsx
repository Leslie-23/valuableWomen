...
const MembershipForm = () => {
  ...
  const authProviders = [auth, auth1, auth2];

  const onSubmit = async (data) => {
    for (let i = 0; i < authProviders.length; i++) {
      try {
        console.log(`Attempting authentication with auth${i}...`);
        ...
        setEmailSent(true);
        reset();
        console.log(`✅ Success with auth${i}`);
        return; // Exit loop on first success
      } catch (error) {
        console.error(`❌ Error with auth${i}:`, error.message);
